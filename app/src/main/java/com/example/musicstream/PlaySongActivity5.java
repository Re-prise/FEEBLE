...
public class PlaySongActivity5 extends AppCompatActivity {
    ...
    SongCollection5 songCollection5 = new SongCollection5();
    SongCollection5 randomCollection5 = new SongCollection5();
    ImageButton loopbtn;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        repeatFlag = false;
        shuffleFlag = false;
        shuffleList = Arrays.asList(songCollection5.albumplaylistplaylistsong);
        ...
        String nameTitle = songData.getString("songname5");
        Log.d("show", "Retrieved title is: " + nameTitle);
        for (int i = 0; i < songCollection5.albumplaylistplaylistsong.length; i++){
            Log.d("show", "Retrieved title is: " + songCollection5.albumplaylistplaylistsong[i].getTitle());
            if (songCollection5.albumplaylistplaylistsong[i].getTitle().equals(nameTitle)){
                Log.d("show", "Retrieved Position is: " + i);
                currentIndex = songCollection5.albumplaylistplaylistsong[i].getId()-1;
                //return;
            }
        }
    ...
    public void displaySongBasedOnIndex(int currentIndex) {
        SongCollection5 SC = new SongCollection5();
        Song song;
        if (shuffleFlag){
    ...
    public void playPrev (View view){
        currentIndex = songCollection5.getPrevSong(currentIndex);
    ...
    public void playNext (View view) {
        if(shuffleFlag){
            ...
            currentIndex = songCollection5.getNextSong(currentIndex);
        }
    ...
    public void shuffleSong(View view) {
        if (shuffleFlag){
            ...
            songCollection5 = new SongCollection5();
        } else {
            shufflebtn.setImageResource(R.drawable.shuffle_icon_filled);
            shuffleList = Arrays.asList(randomCollection5.albumplaylistplaylistsong);
            Collections.shuffle(Arrays.asList(randomCollection5.albumplaylistplaylistsong));
            ...
            for (int s = 0; s < shuffleList.size(); s++){
                ...
                if (title == shuffleList.get(s).getTitle()){
                    ...
                    shuffleList.toArray(randomCollection5.albumplaylistplaylistsong);
                }
            }
    ...
    public Song getRandoSong(int currentSongId) {
        ...
        return randomCollection5.albumplaylistplaylistsong[currentSongId];
    }
    ...
    public void reshuffleSong() {
        ...
        for (int s = 0; s < shuffleList.size(); s++){
            ...
            if (title == shuffleList.get(s).getTitle()){
                Log.d("shuffle1", "located! song position is " + s);
                shuffleList.toArray(songCollection5.albumplaylistplaylistsong);
            }
        }
    }
    ...
    public int getRandomSong(int currentSongIndex){
        ...
        Log.d("temasek","The index in the arris :" + randomCollection5.albumplaylistplaylistsong.length);
        if (currentSongIndex >= randomCollection5.albumplaylistplaylistsong.length-1){return currentSongIndex;}
    ...
    public void teleportToHome(View view){startActivity(new Intent(PlaySongActivity5.this,MainActivity.class));}
    //onClick from Toolbar(settings) to Settings Activity
    public void teleportToSettings(View view){startActivity(new Intent(PlaySongActivity5.this,SettingsActivity.class));}
    //onClick from Toolbar(collections) to Collections Activity
    public void teleportToCollections(View view){startActivity(new Intent(PlaySongActivity5.this,CollectionsActivity.class));}
